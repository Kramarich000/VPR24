label D1P2_PERED_PAROY:
    ...
    s "Хорошо, а конкретно что бы ты хотел узнать?"
...
label D1P2:
    ...
    show o_v at center
    with dissolve
    o_v "Ну что же, ребята давайте знакомиться"
    ...
    o_v "Во-первых необходима, как я уже говорила максимально возможная посещаемость, во-вторых это какая-никакая активность на практических занятиях"
    ...
    o_v "У вас были СКНФ и СДНФ?"
    a "Конечно это одна из первых тем"
    o_v "Так это уже хороший разговор, давайте с СКНФ и СДНФ начнем, а там уже посмотрим по делу"
    "~ Хм...СКНФ и СДНФ так она сказала? Впервые слышу{w}, ну да ладно, посмотрим что это такое ~"
    o_v "Так я сейчас разберу простейший пример с помощью таблицы, а потом вызову одного человека к доске чтобы он нашел СКНФ с помощью преобразований"
    o_v "Итак, решаем: (x v y) -> (x v z) v y, здесь у нас три переменные будут, так что в таблице соотвественно тоже три переменные"
    window hide
    pause
    window show
    o_v "Так, и вот так находится СКНФ и СДНФ с помощью таблицы, а теперь я вызову одного желающего к доске"
    ...
    menu:
        "Поднять руку":
            ...
            pause
            window show
            s "Простите, сразу такой вопрос, а как импликация раскрывается?"
            o_v "Вот тебе таблица с формулами, с прошлого года у вас должны были они остаться в памяти, но раз уж у вас не было такого, то прощаю"
        ...
        "Воздержаться":
            ...
            stud "Ахахаха"
            g "Так СКНФ?"
            o_v "Да решить с помощью элементарных преобразований"
            window hide
            pause
            pause
            window show
            o_v "Так ну и какой же ответ?"
            g "У меня выходит единица"
            o_v "У меня тоже, молодец"
            o_v "Вот так чтобы все решали, четко{w}, быстро{w}, а главное - точно"
            o_v "У кого-нибудь есть вопросы?"
            stud "..."
            o_v "Ну в таком случае поднимаем ставки...следующий пример"
    window hide
    pause
    window show
...
label D1T2:
    # scene а я не ебу какая тут ибо только одиночка по логике подходит но он то не один короче не ебу надо че нить придумать
