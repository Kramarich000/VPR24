...
label start:
    ...
    author "{glitch}{sc}...ГДЕ ЕСТЬ ЖИЗНЬ{/sc}{/glitch}"
    ...
    s "Эм всем привет!{w} Как вы поняли меня зовут Стас{w} и это моя история"
    s "Недавно мне стукнуло 19 лет, так что родители сказали: либо ты учишься{w} либо идешь в армию"
    ...
    s "Я решил попробовать свои силы в рамках новой образовательной программы под названием:{w} \"Школа X\""
    s "И вроде по началу мне даже нравилось:{w} я ходил на пары{w}, работал в команде{w}, да и товарищи были рядом..."
    s "Но что-то не то..."
    s "..."
    ...
    s "Иван Васильич{w}, у меня к вам есть разговор"
    i "В чем дело?"
    ...
    i "Может быть я могу чем-то помочь?{w} У меня есть знакомый психолог хороший, можешь ему высказаться, тебе полегчает"
    s "Нет, простите со мной все в порядке{w}, меня просто...."
    s "...напрягает{w} что на меня давят родители{w} и{w} сейчас..."
    s "И сейчас я в сложной жизненной ситуации{w}, пожалуйста войдите в мое положение прошу вас"
    i "Эх...{w} Станислав{w} ты хорошо учишься так что я сделаю исключение для тебя"
    s "Спасибо!"
    ...
    with dissolve
    play bgs "sounds/zvuki-na-ulice-goroda.mp3"
    s "Черт{w}, надо спешить"
    s "Опаздываю"
    stop bgs
    show DGTU_BG_INSIDE
    with dissolve
    play bgs "sounds/turniket.mp3"
    s "Так пропуск{w}, где же ты"
    s "Ага нашел!"
...
label DGTU_OUTSIDE:
    ...
    hide BLACK_SCREEN
    with dissolve
    g "Боря быстрей{w}, сейчас опоздаем{w}, нас Поркш убьёт!"
    ...
    with dissolve
    d "Дарова!{w} Че вы тут стоите?"
    g "Как-никак тебя ждем"
    d "А пара скоро?"
    b "Да вот через..."
    d "Да пофиг, я курить. {w}Если что я опоздаю!"
    b "Ага{w}, пофиг{w}, у тебя мало того что долг, так еще и посещаемость решил \"исправить\"?"
    g "Вот именно!{w} Tак что никаких курить." 
    g "Пойдем.{w} У нас теорвер"
    d "Ладно... {w}На ковер к Поркшу не особо хочется..."
    hide DIVAN_SMILE
    hide NIKITA_OUTSIDE
    show NIKITA_OUTSIDE at left2
    with dissolve
    n "Всем здарова челики!{w} A че вы не на паре?"
...
label in_DGTU:
    ...
    show ZHENYA_OUTSIDE
    with dissolve
    z "Погодите!{w} Я карту найти не могу"
    # Спрайт Арины
...
label NA_PARE_PORKHA:
    ...
    p "{glitch}{sc}Иначе следующего раза не будет{/sc}{/glitch}"
