init:
    ...
    # дефолт: b - жирный, i - курсив, \n - конец строки, u - подчеркивание, color=#FFFFFF - задать цвет
...
label NA_PARE_PORKHA:
    ...
    menu:
        ...
        "Внимательно слушать":
            $ uspevaemost += 1
            nvl_p "{b}Теория вероятностей{/b} — это наука, которая изучает мир случайностей и пытается их предсказать. Здесь встречаются такие понятия, как «события» и «вероятности», у которых, в свою очередь, есть свои свойства и операции — о них мы поговорим чуть позже."
            ...
            nvl_p "Существует много разных видов и классификаций событий, но на этой лекции мы остановимся на основных четёрых:"
            ...
            stud "Ахахахах"
            nvl_p "{b}Случайные{/b} — те, которые могут произойти, а могут и не произойти. Например, если мы бросаем игральный кубик, то не можем с уверенностью сказать, что выпадет число 2."
            nvl_p "{b}Несовместимые{/b} — те, которые исключают друг-друга. Например, при подбрасывании монетки может выпасть либо орёл, либо решка — оба одновременно они выпасть не могут."
            ...
            nvl_p "{b}Вероятность{/b} — это число, которое обозначает шанс возникновения события. Например, вероятность выигрыша в лотерею может составлять 1 к 1 000 000."
